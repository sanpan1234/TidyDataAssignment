...
***The course project*** required us to combine the above variable values with the corresponding activity and subject (identified by numbers) and then separate out the mean and standard deviations values only into a separate dataset.

The next level of processing required us to calculate average values at group levels where each group is a unique combination of an activity and a subject. The final variables are all mean values at the group level:
```
**Grouping Variables**
"activity_name"
"subject_num"
